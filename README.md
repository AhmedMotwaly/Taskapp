...
Live Application: https://autobuyguard.store
...
Price Volatility: Products frequently change prices, and limited-time deals are easy to miss
Stock Availability: Popular items sell out quickly, requiring constant manual monitoring
...
Cost optimization at ~$43/month for complete infrastructure
...
PlatformStatusSpecial FeaturesAmazon✅ ActiveAnti-bot evasion, dynamic pricingZalando✅ ActiveComplex variant detection (v13.1)eBay✅ ActiveAuction monitoringMediaMarkt/Saturn✅ ActiveGerman electronics retailerAdidas✅ ActiveShell page detectionNike✅ ActiveAnti-bot measuresRossmann✅ ActiveGerman drugstore chainUniversal Scraper✅ ActiveFallback for unsupported sites
💎 Pricing Tiers
FeatureFreePro (€5/mo)Ultra (€10/mo)Check Frequency24 hours12 hours6 hoursDeal Sniper Items110UnlimitedRestock Sniper Items110UnlimitedSMS Alerts❌✅✅Browser Extension✅✅✅Priority Support❌✅✅
...
typescriptNext.js 14 (App Router)     // Server-side rendering, file-based routing
...
javascriptNode.js + Express.js        // RESTful API server
...
yamlGitHub Actions              # CI/CD automation
...
SubnetTypeCIDRAZPurposeTaskApp-Public-1aPublic10.0.1.0/24eu-central-1aALB, NAT GatewayTaskApp-Private-1aPrivate10.0.3.0/24eu-central-1aEC2, RDSTaskApp-Public-1bPublic10.0.2.0/24eu-central-1bALBTaskApp-Private-1bPrivate10.0.4.0/24eu-central-1bEC2, RDSTaskApp-Public-1cPublic10.0.5.0/24eu-central-1cALBTaskApp-Private-1cPrivate10.0.6.0/24eu-central-1cEC2
Route Tables (3 total)

TaskApp-Public-RT: Routes to Internet Gateway
TaskApp-Private-RT: Routes to NAT Gateway for outbound internet
Main Route Table: Local VPC traffic

Security Groups
Security GroupInbound RulesPurposeTaskApp-ALB-SGHTTP (80), HTTPS (443) from 0.0.0.0/0Allow public web trafficTaskApp-WebServer-SGHTTP (80) from ALB-SG, SSH (22) from specific IPEC2 instancesTaskApp-Database-SGMySQL (3306) from WebServer-SGRDS access control
Database Layer
RDS MySQL
...
Manual Rollback:
bash# Option 1: Via GitHub Actions
...
Single NAT Gateway (vs. 3 for high availability)
...
Problem:
javascript// This code caused errors:
...
typescript// page.tsx (Server Component)
...
Standardized to https://www.autobuyguard.store everywhere:
...
Added Route 53 alias record for both root and www
Rebuilt Amplify app to inject updated env vars
...
Problem:
Scraper returned shell pages with no product data:
javascript{
  title: "adidas",
  price: "€0.00",
...
Solution Evolution:
javascript// v1.0: Basic scraping (failed)
...
Problem:
New EC2 instances launched but immediately marked unhealthy by ALB.
Root Cause: User Data script had syntax error, Docker container crashed silently.
Solution:

Added comprehensive logging to User Data:
...
Verified health check endpoint in backend:

javascriptapp.get('/health', (req, res) => {
  res.status(200).json({ status: 'healthy' });
});

Increased ALB health check grace period: 30s → 300s
...
NAT Gateway is expensive ($32/month) — consider alternatives
...
Local Development
1. Clone Repository:
bashgit clone https://github.com/AhmedMotwaly/Taskapp.git
cd Taskapp
2. Backend Setup:
bashcd backend
npm install
...
Frontend (.env.local):
bashNEXTAUTH_URL=http://localhost:3000
...
🔗 LinkedIn: linkedin.com/in/ahmed-motwaly
🐙 GitHub: @AhmedMotwaly
🌐 Portfolio: autobuyguard.store
Certifications
